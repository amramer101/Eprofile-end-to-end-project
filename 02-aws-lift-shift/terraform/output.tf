...
output "ssh_commands" {
  ...
  value = {
    nginx  = "ssh -i ${aws_key_pair.EC2_Key_Pair} ubuntu@${module.ec2_instance_nginx.public_ip}"
    tomcat = "ssh -i ${aws_key_pair.EC2_Key_Pair} ubuntu@${module.ec2_instance_tomcat.public_ip}"
    mysql  = "ssh -i ${aws_key_pair.EC2_Key_Pair} ubuntu@${module.ec2_instance_mysql.public_ip}"
  }
}
